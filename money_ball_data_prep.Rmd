...
# ...	-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
	...
	mlb$team_batting_1b <- mlb$team_batting_h - mlb$team_batting_2b - mlb$team_batting_3b - mlb$team_batting_hr
	mlb$team_batting_tb <- mlb$team_batting_1b + mlb$team_batting_bb + 
							2 * mlb$team_batting_2b + 3 * mlb$team_batting_3b + 4 * mlb$team_batting_hr +
							mlb$team_baserun_sb - mlb$team_baserun_cs
# ...	_tb does not include _hbp and _cs since those columns have high NA counts
```
...
#			png (filename = paste0(names(mlb[i]), "_%02d.png"), width = 1800, height = 600)
			...
			if (skewness(mlb[,i], na.rm = TRUE) >= 0.9)
			{
				...
				hist(xxx, main = names(mlb[i]), cex.lab = cx_scale, cex.main = cx_scale)
				if (skewness(xxx, na.rm = TRUE) < 0)
				{
					txt_pos <- "topleft"
				}
				else {
				...
				plot(mlb$target_wins  ~ xxx, col = "blue",
					 cex = cx_scale, cex.lab = cx_scale, cex.axis = cx_scale, cex.main = cx_scale,
				 	 ylab = "money_ball Score",
				  	 xlab = names(mlb[i]),
				  	 main = "Independent variable vs. money_ball Score")
				abline(lm(mlb$target_wins ~ xxx))

# ...	-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
# ...	look at residuals from one-variable linear fit			
# ...	-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

				fit <- lm(mlb$target_wins ~ xxx, na.action = na.exclude)
				res <- resid(fit)
				plot (xxx, res,
					  cex = cx_scale, cex.lab = cx_scale, cex.axis = cx_scale, cex.main = cx_scale,
					  ylab = "Residuals",
